...
class OracleServicer(service_pb2_grpc.OracleServicer):
    ...
    def GetSpace(self, request, context):
        ...
        if self.logger:
            self.logger.register_tuner(request.tuner_id)
    ...
    def CreateTrial(self, request, context):
        ...
        if self.logger:
            self.logger.create_trial(trial.hyperparameters.values,
                                     request.tuner_id,
                                     trial.trial_id)
    ...
    def UpdateTrial(self, request, context):
        status = self.oracle.update_trial(request.trial_id,
                                          ...
                                          step=request.step)
        if self.logger:
            self.logger.update_trial(request.trial_id, request.metrics)
...
def start_server(oracle, logger):
    ...
    service_pb2_grpc.add_OracleServicer_to_server(
        OracleServicer(oracle, logger), server)
