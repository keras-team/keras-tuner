...
class UltraBand(Tuner):
    ...
    FIXME:
     ...
     - allows different type of distribution
    """
        ...
        self.epoch_sequence = self.__geometric_seq(self.halving_ratio,
                                                   self.min_epochs,
                                                   self.max_epochs)
        self.model_sequence = list(
            reversed(self.epoch_sequence
                     ))  # FIXME: allows to use another type of sequence
        # clip epoch_sequence to ensure we don't train past max epochs
        self.epoch_sequence[-1] = self.epoch_sequence[-1] - np.sum(
        ...
        for i in range(1, len(self.epoch_sequence) + 1):
            ...
            self.band_costs.append(
                np.dot(self.epoch_sequence[:i], self.model_sequence[:i])
            )  # Note: General form, sepecialize sequences have faster way
    ...
    def search(self, x, y, **kwargs):
        while self.loop_left > 0:
            cprint(
                ...
                (self.epoch_budget_expensed, self.epoch_budget, self.loop_left,
                 self.num_loops), 'blue')

            # Last (fractional) loop
            if self.loop_left < 1:
                # Reduce the number of models for the last fractional loop
                self.model_sequence = self.__scale_loop(
                    self.model_sequence, self.loop_left)
                cprint('|- Scaled models seq %s' % self.model_sequence,
                       'yellow')
            for band_idx, num_models in enumerate(self.model_sequence):
                band_total_cost = 0
                cprint(
                    'Budget:%s/%s - Loop %.2f/%.2f - Bands %s/%s' %
                    (self.epoch_budget_expensed, self.epoch_budget,
                     self.loop_left, self.num_loops, band_idx + 1,
                     self.num_bands), 'green')
                ...
                kwargs['epochs'] = num_epochs
                if self.dry_run:
                    loss_values = model_instances = np.random.rand(
                        num_models)  # real training instead
                else:
                    loss_values = []
                ...
                for step, num_models in enumerate(
                        self.model_sequence[band_idx + 1:]):
                    ...
                    band_models = self.__sort_models(
                        model_instances, loss_values
                    )  # Bogus replace 2nd term with the loss array
                    band_models = band_models[:num_models]  # halve the m odels

                    # train
                    cprint(
                        '|- Training %s models for an additional %s epochs' %
                        (num_models, num_epochs), 'yellow')
                    kwargs['epochs'] = num_epochs
                    if self.dry_run:
                        loss_values = model_instances = np.random.rand(
    ...
    def __scale_loop(self, seq, ratio):
        ...
        scaled_seq = np.round(np.asarray(seq) * ratio).astype(
            int)  # floor: don't want to go budget
        return scaled_seq
    ...
    def __sort_models(self, models, loss_values):
        ...
        indices = np.argsort(
            loss_values)  # recall loss is decreasing so use asc
        sorted_models = []
        for idx in indices:
