...
class GridSearch(Tuner):
    ...
    def __init__(self, model_fn, objective, **kwargs):
        """
        Args:
            epoch_budget (int): How many epochs to spend hyper-tuning. Default
                3171
            ...
            display_model (str): base: cpu/single gpu version,
               multi-gpu: multi-gpu, both: base and multi-gpu. default (Nothing)

            num_gpu (int): number of gpu to use. Default 0
            gpu_mem (int): amount of RAM per GPU. Used for batch size
                calculation

            local_dir (str): where to store results and models. Default results/
            gs_dir (str): Google cloud bucket to use to store results and model
                (optional). Default None

            dry_run (bool): do not train the model just run the pipeline.
                Default False
            max_fail_streak (int): number of failed model before giving up.
                Default 20
        """
    ...
    def tune(self, x, y, **kwargs):
        ...
        if required_num_epochs > self.state.remaining_budget:
            warning(
                "GridSearch epoch budget of %d is not sufficient to explore \
