...
# DRY_RUN: True don't train the models,  DRY_RUN: False: train models.
DRY_RUN = False
...
def model_fn():
    ...
    L2_ACTIVATION = Choice('hidden activation', [
                           'relu', 'tanh'], group="hidden_layers")
    L2_OPTIONAL = Boolean('use hidden layer', group="hidden_layers")
    # Last layer
    LL_UNITS = Fixed('ouput dims', 1, group="output")
    LL_ACTIVATION = Choice('output activation', [
                           'sigmoid', 'tanh'], group="output")
    # Compile options
...
METRIC_TO_REPORT = [('loss', 'min'), ('val_loss', 'min'),
                    ('acc', 'max'), ('val_acc', 'max')]
hypermodel = RandomSearch(model_fn, epoch_budget=90, max_epochs=10, dry_run=DRY_RUN,
                          project="kerastuner-demo", architecture="MLP", info={'model_type': 'MLP'}, metrics=METRIC_TO_REPORT)
hypermodel.summary()
