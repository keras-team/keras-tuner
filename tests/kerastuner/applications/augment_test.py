...
def test_transforms_search_space():
    if applications.HyperImageAugment is None:
        return
    hm = applications.HyperImageAugment(input_shape=(32, 32, 3))
    # Default choice
...
def test_input_requirement():
    ...
    with pytest.raises(ValueError, match=r'.*must specify.*'):
        hm = applications.HyperImageAugment()

    hm = applications.HyperImageAugment(input_shape=(None, None, 3))
    model = hm.build(hp)
    assert model.built
...
def test_hyperparameter_override_fixed_aug():
    ...
    hm = applications.HyperImageAugment(input_shape=(32, 32, 3), augment_layers=0)
    hm.build(hp)
